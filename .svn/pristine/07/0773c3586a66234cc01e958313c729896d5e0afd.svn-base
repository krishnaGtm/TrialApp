   ...
   public class AESEncryption
    {
        ...
        public static string EncryptAesTest(string data, string password)
        {
            try
            {
                var SAP = WinRTCrypto.SymmetricKeyAlgorithmProvider.OpenAlgorithm(SymmetricAlgorithm.AesEcbPkcs7);
                var HAP = WinRTCrypto.HashAlgorithmProvider.OpenAlgorithm(HashAlgorithm.Sha512);
                ...
                Hash_AES.Append(WinRTCrypto.CryptographicBuffer.CreateFromByteArray(System.Convert.FromBase64String(strBase64)));
                byte[] temp;
                WinRTCrypto.CryptographicBuffer.CopyToByteArray(Hash_AES.GetValueAndReset(), out temp);
                Array.Copy(temp, 0, hash, 0, 16);
                //Array.Copy(temp, 0, hash, 15, 16);
                var AES = SAP.CreateSymmetricKey(WinRTCrypto.CryptographicBuffer.CreateFromByteArray(hash));
                var Buffer = WinRTCrypto.CryptographicBuffer.CreateFromByteArray(Encoding.UTF8.GetBytes(data));
                encrypted = WinRTCrypto.CryptographicBuffer.EncodeToBase64String(WinRTCrypto.CryptographicEngine.Encrypt(AES, Buffer, null));
                return encrypted;
            }
            catch (Exception ex)
            {
