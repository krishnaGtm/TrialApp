...
namespace TrialApp.Services
{
    ...
    public class TrialService
    {
        ...
        public async Task<bool> UpdateTrialAndObservationData(List<Trial> listofCheckedTrials)
        {
            using (var db = new SQLiteConnection(DbPath.GetTransactionDbPath()))
            {
                db.BeginTransaction();
                foreach (var trial in listofCheckedTrials)
                {
                    db.Execute("UPDATE Trial SET StatusCode = 20 WHERE EZID = ?", trial.EZID);
                    db.Execute(
                        @"UPDATE ObservationApp SET Modified = 'false' WHERE EZID in (SELECT OBS.EZID FROM ObservationApp AS OBS JOIN Relationship AS R ON R.EZID2 = OBS.EZID JOIN Trial T on T.EZID = R.EZID1 WHERE T.EZID =?)",
                        trial.EZID);
                    db.Execute(
                        @"UPDATE ObservationApp SET Modified = 'false' WHERE EZID =?",
                        trial.EZID);
                    db.Execute(
                        @"UPDATE TrialEntryApp SET Modified = 'false', NewRecord = 'false' WHERE EZID in (SELECT TEA.EZID FROM TrialEntryApp AS TEA JOIN Relationship AS R ON R.EZID2 = TEA.EZID JOIN Trial T on T.EZID = R.EZID1 WHERE T.EZID =?)",
                        trial.EZID);
                    //await LogMessage.Log(string.Concat(trial.EZID.ToString(), " : ", trial.TrialName));
                }
                db.Commit();
